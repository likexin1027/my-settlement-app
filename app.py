...
def normalize_platform(s):
    if pd.isna(s): return ""
    x = str(s).strip().lower()
    if "b站" in x or "bilibili" in x or "哔哩" in x: return "B站"
    if "小红书" in x or "red" in x: return "小红书"
    if "视频号" in x: return "视频号"
    if "抖音" in x or "douyin" in x: return "抖音"
    return s
...
def load_default_mapping():
    try:
        try: mdf = pd.read_csv("奖励金额t.csv")
        except: mdf = pd.read_csv("奖励金额t.csv", encoding="gbk")
        cols = set(mdf.columns)
        if "平台" in cols and "奖励金额" in cols and ("阈值数值" in cols or "阈值标签" in cols):
            out = mdf.copy()
            if "阈值数值" not in cols and "阈值标签" in cols:
                out["阈值数值"] = out["阈值标签"].apply(normalize_label_to_value)
            if "阈值标签" not in cols and "阈值数值" in cols:
...
def value_to_label(v):
    if v >= 1000000: return "≥100w"
    if v >= 500000: return "≥50w"
    if v >= 200000: return "≥20w"
    if v >= 100000: return "≥10w"
    if v >= 50000: return "≥5w"
    if v >= 30000: return "≥3w"
    if v >= 10000: return "≥1w"
    return f"≥{int(v)}"
...
def describe_excel_error(err, filename):
    s = str(err).lower()
    reasons = []
    if "encrypted" in s or "password" in s: reasons.append("文件加密或受保护")
    if "not a zip file" in s or "unsupported file format" in s or "badzipfile" in s: reasons.append("文件损坏或并非标准xlsx/xls")
    if "calamine" in s and ("not installed" in s or "module" in s): reasons.append("缺少读取引擎，请安装python-calamine")
    if "openpyxl" in s and ("styles" in s or "fills" in s): reasons.append("复杂样式导致解析失败，建议重导出或简化样式")
    if filename.endswith(".xls") and ("xlrd" in s or "format" in s): reasons.append(".xls兼容性问题，建议另存为.xlsx后再上传")
    if "filetype" in s or "content-type" in s: reasons.append("扩展名与实际内容不匹配")
    msg = "Excel读取失败"
...
def read_xlsx_robust(bio):
    try: return pd.read_excel(bio, engine="calamine")
    except:
        bio.seek(0)
...
def excellence_bonus(plat, typ, likes, views):
    ...
    if isinstance(typ, str):
        ...
        if "短视频" in typ and likes >= 100000: b += 300.0
        if "短视频" in typ and views >= 2000000: b += 1000.0
    return b
...
def chat_with_ai(user_prompt, context_data):
    try:
        if "DEEPSEEK_API_KEY" not in st.secrets:
            return "错误：未在 Secrets 中配置 DEEPSEEK_API_KEY。"
        ...
        system_prompt = (
            ...
            "1. **计算效能**：关注金额和播放量的关系，识别高性价比作者。\n"
            "2. **数据监控**：直接引用报表中的具体数字，指出播放量与金额不匹配的异常账号。\n"
            "3. **专业表达**：严禁解释名词，直接给出结论。"
        )
        ...
        if cfg is not None:
            ...
            try:
                if n.endswith(".csv"):
                    ...
                    bio = io.BytesIO(data)
                    if n.endswith(".xlsx"):
                        try: mdf = pd.read_excel(bio, engine="calamine")
                        except:
                            bio.seek(0)
    ...
    with tabs[0]:
        ...
        if name.endswith(".csv"):
            ...
            try:
                data = uploaded.read()
                bio = io.BytesIO(data)
                if name.endswith(".xlsx"): df = read_xlsx_robust(bio)
                elif name.endswith(".xls"): df = pd.read_excel(bio, engine="xlrd")
                else:
                    bio.seek(0)
        ...
        result = kept[["渠道", "账号名称", "播放量", "点赞", "作品类型", "基础奖励", "限时奖励", "优秀奖励", "总奖励", "是否计入结算"]].copy()
        ...
        if "summary_data" in st.session_state and not st.session_state["summary_data"].empty:
            ...
            st.info("💡 数据正在处理或为空，处理完成后 AI 助手将就绪。")
